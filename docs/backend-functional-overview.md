...
- возвращается в `GET /api/v1/auth/me`
- возвращается в `GET /api/v1/users`
- возвращается в `GET /api/v1/users/{id}`
- поддерживается в create/update пользователя
- валидируется как строка ровно из 12 цифр
- должен быть уникальным, если заполнен
...
Календарная логика по `dateFrom/dateTo`:
- backend возвращает не только поездки, у которых старт лежит внутри периода
- используется логика пересечения диапазонов:
  - `plannedStartDateTime <= requestedDateTo`
  - `plannedEndDateTime >= requestedDateFrom`

Это значит:
- поездка, начавшаяся в прошлом месяце и продолжающаяся в текущем, попадет в выборку
- поездка, начавшаяся в текущем месяце и заканчивающаяся в следующем, тоже попадет
- поездка полностью вне диапазона не вернется

Эта логика работает через существующие list endpoints и для `ADMIN`, и для `EMPLOYEE`.
